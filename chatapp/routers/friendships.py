...
def create_new_friendship(friendship_data: dict, db: Session = Depends(get_db)):
    return create_friendship(db, friendship_data)

@router.get("/friendships/{friendship_id}")
def get_single_friendship(friendship_id: int, db: Session = Depends(get_db)):
...
def get_all_friendships(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    return get_friendships(db, skip, limit)

@router.put("/friendships/{friendship_id}")
def update_existing_friendship(friendship_id: int, friendship_data: dict, db: Session = Depends(get_db)):
...
def delete_existing_friendship(friendship_id: int, db: Session = Depends(get_db)):
